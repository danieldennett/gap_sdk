...
static inline void cl_pop_cluster_task(struct cluster_driver_data *data)
{
    ...
    if(task->stack_allocated)
    {
        ...
        pmsis_l1_malloc_free(task->stacks, task->nb_cores * task->stack_size);
        PRINTF("Free %p %d\n", task->stacks, task->stack_size);
        task->stacks = NULL;
        task->stack_size = 0;
...
static inline int __pi_send_task_to_cl(struct pi_device *device, struct pi_cluster_task *task)
{
    ...
    if(!task->stacks)
    {
        task->stacks = pmsis_l1_malloc(task->nb_cores * task->stack_size);
        PRINTF("Malloc stack %p %d\n", task->stacks, task->nb_cores * task->stack_size);
        if (task->stacks == NULL)
        {
            pmsis_mutex_release(&data->task_mutex);
            return -1;
